...
const Card = ({data}) => {
  ...
  const cardScrollTrigger = {trigger: ()=>q('.card-bg'), start: 'top top', end: '120% top', scrub: true}

  useEffect(()=>{
    gsap.fromTo(q('.card'), {x: 120, opacity: 0}, {x: 0, opacity: 1, stagger: 0.33, duration: 2, onStart: ()=> setIsLoading(true), onComplete: ()=> setIsLoading(false)})
  },[])
  ...
  useEffect(()=>{
    const animation1 = gsap.to(q('.btn'), { y: -100, opacity: 0, duration: 0.7, scrollTrigger: {
      trigger: ()=>q('.card-bg'),
      start: '35% top',
      end: 'bootom center',
      scrub: true
    } })
    const animation2 = gsap.fromTo(q('.time'), { opacity: 0, y: 100 }, { opacity: 1, y: 0, duration: 0.7, scrollTrigger: {
      trigger: ()=>q('.card-bg'),
      start: 'top center',
      end: 'bootom center',
    } })
  ...
  return (
    <div className="section" ref={el}>
      <div className="card" data-text={`url(../../assets/images/${data.background})`} onMouseEnter={onEnter} onMouseLeave={onLeaver} onClick={pllayMusic}>
        <div className='card-shadow' style={{ background: `url(/src/assets/images/${data.background}) no-repeat`, backgroundSize: 'cover'}}></div>
        <div className="card-role" style={{ background: `url(/src/assets/images/${data.role}) no-repeat`, backgroundSize: 'cover' }}>
        </div>
        <div className="card-bg" style={{ background: `url(/src/assets/images/${data.background}) no-repeat`, backgroundSize: 'cover'}}></div>
        <div className="time">
          <h1>{data.day}</h1>
      ...
      <audio src={`/src/assets/music/${data.music}`} ref={audioEl}></audio>
    </div>
  )
}
...
const Cars = () => {
  return (
    <>
      {
        carData.map(item => <Card data={item} key={item.id}/>)
      }
    </>
  )
}
